...
function Dashboard({ onLogout }) {
  ...
  const discardComponent = (id, name) => {
    const isConfirmed = window.confirm(
      `"${name}" adlı donanımı sistemden tamamen silmek istediğinize emin misiniz?\n\nBu işlem geri alınamaz.`
    );
    if (isConfirmed) {
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="mb-8 flex flex-col md:flex-row justify-between items-center gap-4">
            ...
            <div className="relative">
              <input
                ...
                className="pl-4 pr-10 py-2.5 border border-gray-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-blue-500 w-72 shadow-sm bg-white text-slate-700"
              />
          ...
          <div className="bg-white p-6 rounded-2xl shadow-sm mb-8 border border-gray-200">
            <h3 className="text-lg font-bold text-slate-700 mb-4 flex items-center gap-2">
              <span>⚙️</span> Yeni Komponent Kaydı
            </h3>
            <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
              <input
                type="text"
                placeholder="Devre Elemanı Adı"
                value={newModule.name}
                onChange={(e) => setNewModule({ ...newModule, name: e.target.value })}
                className="col-span-2 px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
              <input
                type="text"
              ...
              <input
                type="number"
                placeholder="İlk Stok"
                value={newModule.stock}
                onChange={(e) => setNewModule({ ...newModule, stock: e.target.value })}
                className="px-4 py-2 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
              <input
                type="number"
            ...
            <div className="mt-5 flex justify-end">
              <button
                ...
                className="bg-slate-900 text-white px-6 py-2.5 rounded-lg hover:bg-slate-800 transition-colors font-semibold shadow-md"
              >
                + Sisteme İşle
              </button>
            </div>
                  ...
                  {filteredHardware.map((item) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="p-5">
                        <div className="flex items-center justify-center bg-slate-100 p-1.5 rounded-lg border border-slate-200 w-max mx-auto shadow-inner">
                          <button
                            ...
                            className="w-8 h-8 flex items-center justify-center rounded-md bg-white text-red-500 hover:bg-red-50 hover:text-red-600 font-bold text-lg shadow-sm border border-slate-200 transition-all active:scale-95"
                          >
                            -
                          </button>
                          <span className={`font-bold w-14 text-center text-lg tracking-wide ${item.stock < 10 ? "text-red-600" : "text-slate-800"}`}>
                            {item.stock}
                          </span>
                          <button
                            onClick={() => updateStock(item.id, 1)}
                            className="w-8 h-8 flex items-center justify-center rounded-md bg-white text-emerald-500 hover:bg-emerald-50 hover:text-emerald-600 font-bold text-lg shadow-sm border border-slate-200 transition-all active:scale-95"
                          >
                            +
                          </button>
                        </div>
                      ...
                      <td className="p-5 text-center">
                        <button
                          onClick={() => discardComponent(item.id, item.name)}
                          className="text-red-500 hover:text-red-700 hover:bg-red-50 px-3 py-1.5 rounded transition-colors text-sm font-medium border border-transparent hover:border-red-200"
                        >
                          Sistemden Sil
                        </button>
                      </td>
